import os
from dotenv import load_dotenv
from openai import OpenAI

from schemas.travel_destination import TouristSpotExtract

load_dotenv()

openai = OpenAI(
    api_key = os.environ.get("OPENAI_API_KEY")
)

# system prompt 작성
SYSTEM_PROMPT = """
너는 한국 여행지 메타데이터 추출기다.
입력된 본문 텍스트를 읽고 TouristSpotExtract 스키마에 맞는 값만 추출하라.

중요 규칙:
1. companions는 반드시 다음 값만 사용:
   - family
   - couple
   - friends
   - solo
   - parents

2. 원문 표현을 표준값으로 정규화:
   - 연인, 커플, 여자친구, 남자친구, 애인, 데이트 -> couple
   - 가족, 아이와, 아이랑, 아기와, 유아 동반 -> family
   - 부모님, 어머니, 아버지, 엄마, 아빠 모시고 -> parents
   - 친구, 친구들, 동기, 지인들과 -> friends
   - 혼자, 솔로, 나홀로, 1인 여행 -> solo

3. companions는 "누구와 가기 좋다", "추천한다", "잘 어울린다", "적합하다"처럼
   긍정적인 추천 의미가 본문에 있을 때만 추가한다.

4. 단순히 함께 갔다는 사실만으로는 companions에 추가하지 않는다.
   예:
   - "친구와 갔다" -> friends 추가하지 않음
   - "연인과 방문했다" -> couple 추가하지 않음

5. categories는 반드시 다음 값만 사용:
   - healing
   - nature
   - activity
   - culture
   - food
   - photo

6. 카테고리 정규화 예시:
   - 힐링, 여유, 조용함, 휴식 -> healing
   - 자연, 바다, 숲, 산, 풍경 -> nature
   - 체험, 물놀이, 액티비티, 레저 -> activity
   - 전시, 역사, 박물관, 문화 -> culture
   - 맛집, 먹거리, 음식 -> food
   - 사진, 포토존, 인생샷 -> photo

7. description는 반드시 관광지 설명 내용을 다음과 같이 간결하게 요약:
   - 2~3문장으로 작성하세요.
   - 최대 200자 이내로 제한하세요.
   - 주요 특징, 핵심 매력을 반드시 포함하세요.
   - 불필요한 수식어, 홍보 문구, 반복 표현은 제거하세요.
   - 자연스럽고 읽기 쉬운 한국어로 작성하세요.
   - 원문에 없는 정보는 추가하지 마세요.
   - 설명 없이 요약 결과만 출력하세요.

8. 원문에 근거가 없는 값은 넣지 말 것.
9. 본문에 없는 내용은 절대 추측하지 말 것
10. 본문에서 근거를 찾을 수 없는 필드는 빈값으로 둘 것
11. 문자열 필드에 정보가 없으면 null 반환
12. 리스트 필드에 정보가 없으면 빈 리스트 [] 반환
13. JSON 외 다른 문장은 출력하지 말 것.
"""

# 관광지를 TouristSpotExtract에 맞게 추출하는 함수
def extract_tourist_spot(text: str):
   """여행지를 원하는 데이터 모델 형태로 반환하는 함수

   크롤링한 관광지 데이터를 넣으면 openai LLM을 활용하여 TouristSpotExtract 형태의 데이터를 반환함

   TouristSpotExtract{
      name: 관광지명
      companions: 누구랑 가기 좋은지 (예: 가족, 커플, 친구, 혼자)
      categories: 카테고리 (힐링, 자연, 액티비티 등)
      address: 관광지 주소
      description: 관광지 소개 텍스트
      spot_type: 구분 (축제, 관광지, 체험, 카페 등)
      popularity_score: 인기도 점수 (검색 결과/리뷰 수 기반)
   }

   Args:
        text (str): 크롤링한 해당 관광지 데이터

   Returns:
        TouristSpotExtract: TouristSpotExtract 형태의 관광지 데이터
   """
    
   completion = openai.beta.chat.completions.parse(
   model = "gpt-4o-mini",
   messages = [
      {
         "role": "system", 
         "content": SYSTEM_PROMPT
      },
      {
         "role": "user",
         "content": f"다음 관광지 본문에서 정보를 추출해줘: \n\n{text}"
      },
   ],

   response_format = TouristSpotExtract,
   )

   return completion.choices[0].message.parsed

...
5. description는 반드시 관광지 설명 내용을 다음과 같이 간결하게 요약:
   - 2~3문장으로 작성하세요.
   - 최대 200자 이내로 제한하세요.
...
6. 원문에 근거가 없는 값은 넣지 말 것.
7. 본문에 없는 내용은 절대 추측하지 말 것
8. 본문에서 근거를 찾을 수 없는 필드는 빈값으로 둘 것
9. 문자열 필드에 정보가 없으면 null 반환
10. 리스트 필드에 정보가 없으면 빈 리스트 [] 반환
11. JSON 외 다른 문장은 출력하지 말 것.
"""
